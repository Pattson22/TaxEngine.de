...
| ELSTER submission | ✅ | 🔶 real `cffi` binding to the actual ERiC library, verified end-to-end (`EricCheckXML()` passes cleanly for wage/capital/rental/self-employment/children income plus donations, all together); `xml_builder.py` maps every income/deduction type this project supports (church-tax-paid-directly and the KOMPRIMIERT cover sheet remain open, each for a specific documented reason — see `docs/ELSTER_ERIC_INTEGRATION.md`); not yet wired into the FastAPI app (`NativeEricClient` belongs in a separate worker); still blocked on a registered `HerstellerID` and each filer's Finanzamt BuFa-Nummer |
